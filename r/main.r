  ...
  speciesExperiment = function(speciesData){
    ...
    all_with_clim = all_with_clim[completecases,]
    ...
    model_noprior = buildSDM(all_with_clim) 
    ...
    prior = buildPrior('sigmoid', tminEnvCol = 'bio1', tmaxEnvCol = 'bio1', tmin, tmax)
  
      ## train model with prior
    model_withprior = buildSDM(all_with_clim, prior = prior)
    
    plot(model_withprior, data=F, jitter=0, prior=T)
