...
# transform names for consistency (lowercase, remove underscore)
...
# count species without physiology information
...
# lambda function for writing to failure log
.FAILFILE = function(species, reason, file){
  write(sprintf("%s\t%s", species, reason), append=T, file=file)
}

# lambda function for execution on each species in the dataset, main model.
# TODO: Bring this out into a file. 
speciesExperiment = function(speciesData){
  ...
  # and plot current distribution recovery statistics (ROC)
  ...
  model_maxent = buildMaxEnt(split$train) # defined in SDM.r
  maxent_aucroc = maxent_auc_roc(model_maxent, testCovs, testLabels)  # defined in SDM.r
  ...
  ## test model with future climate predictions 
  ...
  if(PARAMS$predict){
    ...
    comparePredictions(c(subset(rastnoprior, 1), subset(rastwithprior, 1)), 
                       ...
                       arrows=FALSE) # defined in future.R
    dev.off()
  ...
  ## Free memory used for this model run
  gc(verbose=T)
...
if(!is.null(PARAMS$chunksize)){
  ...
  # normal non-chunked run of all species sequentially
